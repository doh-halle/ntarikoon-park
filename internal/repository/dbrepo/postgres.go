...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone_number, r.start_date, r.end_date, r.apartment_id,
...
func (m *postgresDBRepo) UpdateReservation(u models.Reservation) error {
	...
	query := `update reservations set first_name = $1, last_name = $2, email = $3, phone_number = $4, updated_at = $5
				where id = $6`
...
func (m *postgresDBRepo) AllApartments() ([]models.Apartment, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	var apartments []models.Apartment

	query := `select id, apartment_name, created_at, updated_at from apartments order by apartment_name`
...
func (m *postgresDBRepo) GetRestrictionsForApartmentByDate(apartmentID int, start, end time.Time) ([]models.ApartmentRestriction, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	var restrictions []models.ApartmentRestriction

	query := `select id, coalesce(reservation_id, 0), restriction_id, apartment_id, start_date, end_date
			from apartment_restrictions where $1 < end_date and $2 >= start_date 
			and apartment_id = $3`

	rows, err := m.DB.QueryContext(ctx, query, start, end, apartmentID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var r models.ApartmentRestriction
		err := rows.Scan(
			&r.ID,
			&r.ReservationID,
			&r.RestrictionID,
...
func (m *postgresDBRepo) InsertBlockforApartmentByID(id int, startDate time.Time) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `insert into apartment_restrictions (start_date, end_date, apartment_id, restriction_id,
...
func (m *postgresDBRepo) DeleteBlockforApartmentByID(id int) error {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()

	query := `delete from apartment_restrictions where id = $1`
