...
func TestForm_Valid(t *testing.T) {
	...
	isValid := form.Valid()
	if !isValid {
		t.Error("recieved invalid when feedback should be valid")
	}
}
...
func TestForm_Required(t *testing.T) {
	...
	form.Required("a", "b", "c")
	if !form.Valid() {
		t.Error("displays does not have required fields when it does")
	}
}
...
func TestForm_IsEmail(t *testing.T) {
	...
	postedValues = url.Values{}
	postedValues.Add("email", "dh@mail.com")
	form = New(postedValues)

	form.IsEmail("email")
	if !form.Valid() {
		t.Error("got an invalid email when we should not have gotten one")
	}
